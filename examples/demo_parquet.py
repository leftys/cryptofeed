...
def main():
    ...
    # f.add_feed(Binance(channels=[TRADES, L2_BOOK, CANDLES], symbols=['BTC-USDT', 'ETH-BTC'], callbacks={TRADES: TradeParquet(path = S3_PATH), L2_BOOK: BookParquet(path = S3_PATH)}, max_depth = 20))
    # f.add_feed(Binance(channels=[TRADES], symbols=['BTC-USDT', 'ETH-USDT'], callbacks={TRADES: TradeParquet(path = S3_PATH)}))
    ...
    f.add_feed(KuCoin(config = config, channels=[TRADES, L2_BOOK], symbols=['BTC-USDT', 'ETH-USDT'], callbacks={TRADES: TradeParquet(path = S3_PATH), L2_BOOK: BookParquet(path = S3_PATH)}, max_depth = 20))
    # f.add_feed(KuCoin(channels=[TRADES], symbols=['BTC-USDT'], callbacks={TRADES: TradeParquet(path = S3_PATH), L2_BOOK: BookParquet(path = S3_PATH)}, max_depth = 20))
